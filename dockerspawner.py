...
class DockerSpawner(Spawner):
    ...
    async def create_object(self):
        ...
        #We need to break up the volume here
        #python_on_whales Store in different formats
        envs = create_kwargs["host_config"]["Binds"]
        docker_volume = [tuple(env.split(':', 2)) for env in envs]
        #ip = "127.0.0.1"
        #port = "8888"
        # connection database, dynamic query docker container resource
        #session = self.connect_db()
        
        create_kwargs = dict(
            image='jupyter/base-notebook:hub-1.5.1',
            ...
            #support gpu 
            #gpus=f"\"device=1\"",
            #publish=[(ip + ":" + port, 8888)]
            #use docker random ports
            publish_all=True
        )

        # create the container
        # just need to create the container
        res = self.gpu_docker("create", **create_kwargs)
        obj = {
            # Get the container name
            self.object_id_key: str(res.result())
        }
    ...
    #database_url = "mysql+pymysql://root:123456@192.168.1.1:3306/docker_spawner"
    # create database connection
    def connect_db(self):
        try:
            db = "mysql+pymysql://root:123456@192.168.1.1:3306/docker_spawner"
